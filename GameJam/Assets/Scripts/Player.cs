...
namespace DefaultNamespace
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        [Header("Drag controller")] [SerializeField]
        private float _maxGroundDrag;
        ...
        {
            _hp -= 1;
            if (_hp < _countHp) // Ивент для хп худа. Вставить в метод, где отнимается хп
            {
                if (HpChanged != null)
                {
                    HpChanged();
        ...
        private void WallJump()
        {
            if (_isWallRunning)
            {
                ...
                //sidwards wallhop
                if (_isWallRight || _isWallLeft && Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D))
                    _rb.AddForce(-_orientation.up * _forceJump * 1f);
                if (_isWallRight || _isWallLeft && Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D))
                    _rb.AddForce(-_orientation.up * _forceJump * 1f);
