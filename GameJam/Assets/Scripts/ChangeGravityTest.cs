...
public class ChangeGravityTest : MonoBehaviour
{
    ...
    private void CheckGround()
    {
        ...
        RaycastHit _hit;
        if (Physics.Raycast(transform.position, Vector3.up, out _hit, 0.9f, _whatIsGround))
        {
            Debug.DrawRay(transform.position, Vector3.up * 0.9f, Color.green);
            _ground = true;
    ...
    private void Jump()
    {
        if (_ground)
        {
            ...
            {
                _rb.AddForce(Vector3.down * _forceJump);
            }
        }
    ...
    private void VelocityInAir()
    {
        if (!_ground)
        {
            ...
            _rb.AddForce(Vector2.up * _rbDragGravity);
        }
    }
